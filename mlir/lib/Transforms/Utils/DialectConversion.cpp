...
OperationLegalizer::legalize(Operation *op,
  ...
  // If the operation isn't legal, try to fold it in-place if the folding mode
  // is 'OnlyIllegal'. 'Never' will skip this.
  if (config.foldingMode == DialectConversionFoldingMode::OnlyIllegal) {
  ...
  // if the folding mode is 'Unlegalizable'. 'Never' will skip this.
  if (config.foldingMode == DialectConversionFoldingMode::Unlegalizable) {
    if (succeeded(legalizeWithFold(op, rewriter))) {
      LLVM_DEBUG({
        logSuccess(logger, "operation was folded");
        logger.startLine() << logLineComment;
