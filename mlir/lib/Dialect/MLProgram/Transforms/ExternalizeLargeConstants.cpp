...
public:
  ...
  void runOnOperation() override {
    ...
    if (weightCounter > 0 && !outputFilename.empty()) {
      std::string errorMessage;
      auto output = openOutputFile(outputFilename, &errorMessage);
      if (!output) {
        emitError(module.getLoc()) << "failed to open output file: "
                                   << errorMessage;
        weightsModule->erase();
        return signalPassFailure();
      }
      if (failed(writeBytecodeToFile(weightsModule, output->os()))) {
        emitError(module.getLoc()) << "failed to write bytecode to file";
        weightsModule->erase();
        return signalPassFailure();
      }
      output->keep();
