...
Lexer::Lexer(const llvm::SourceMgr &sourceMgr, MLIRContext *context,
  ...
  if (sourceMgr.getNumBuffers() > 1) {
    ...
    if (sourceMgr.getMemoryBuffer(bufferID)->getBufferStart() <=
            sourceMgr.getMemoryBuffer(lastFileID)->getBufferStart() &&
        sourceMgr.getMemoryBuffer(bufferID)->getBufferEnd() >=
            sourceMgr.getMemoryBuffer(lastFileID)->getBufferEnd()) {
      bufferID = lastFileID;
    }
  }
