...
LogicalResult mlir::MlirOptMain(llvm::raw_ostream &outputStream,
  ...
  auto chunkFn = [&](std::unique_ptr<MemoryBuffer> chunkBuffer,
    ...
    return processBuffer(os, std::move(chunkBuffer), sourceBuffer, config,
                         registry, &sourceMgrHandler, threadPool);
  };
  LogicalResult status = splitAndProcessBuffer(
      llvm::MemoryBuffer::getMemBuffer(buffer->getMemBufferRef()), chunkFn,
      outputStream, config.inputSplitMarker(), config.outputSplitMarker());
