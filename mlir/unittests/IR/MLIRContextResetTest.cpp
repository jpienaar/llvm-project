//===- MLIRContextResetTest.cpp - Tests for Resettable MLIRContext
//---------===//
//
...
TEST(MLIRContextResetTest, BasicTypeTransientScopeAndReset) {
  ...
  // 1. Base types created before transient scope.
  ...
  // 2. Base types remain identical under transient scope.
  ...
  // 3. Create transient types in transient scope.
  ...
  // 4. End transient scope back to base state.
  ctx.endTransientScope();
  EXPECT_FALSE(ctx.isInTransientScope());

  // 5. Verify base types are completely preserved.
  EXPECT_EQ(i32Type, builder.getI32Type());
  EXPECT_EQ(f32Type, builder.getF32Type());
  EXPECT_EQ(baseVectorType, VectorType::get({2, 2}, f32Type));

  // 6. Types can be cleanly recreated post-reset.
...
#ifndef NDEBUG
TEST(MLIRContextResetTest, NestedTransientScopeAsserts) {
  MLIRContext ctx;
  ctx.beginTransientScope();
  EXPECT_DEATH(ctx.beginTransientScope(), "already in a transient scope");
}

TEST(MLIRContextResetTest, DialectRegistrationInTransientScopeAsserts) {
  MLIRContext ctx;
  ctx.beginTransientScope();
  DialectRegistry registry;
  EXPECT_DEATH(ctx.appendDialectRegistry(registry),
